...
def remove_emoji(text):
    ...
    text_without_emojis = ''.join(
        word for word in text_without_emojis.split(':') if not any(char.isdigit() for char in word))
...
def manual_testing(news):
    ...
    new_def_test["text"] = new_def_test["text"].apply(english_only)
    new_def_test["text"] = new_def_test["text"].apply(remp_emoticon)
    new_def_test["text"] = new_def_test["text"].apply(remp_abreviation)
    new_def_test["text"] = new_def_test["text"].apply(remove_emoji)
    new_def_test["text"] = new_def_test["text"].apply(wordopt)
    new_def_test["text"] = new_def_test["text"].apply(remove_single_lettre)
    new_x_test = new_def_test["text"]
    new_xv_test = vectorizer.fit_transform(new_x_test)
    pred_KNN = model_gbc.predict(new_xv_test)
